...
Requires the predictions.csv file containing the predicted_fitted intelligibility
for each signal. This should have the following format.

  scene,listener,system,predicted_fitted
...
def main(intel_file_json, prediction_file_csv):
    ...
    with the predicted_fitted intelligibilities

    The true intelligibilities are stored in the CPC1 JSON metadata file
    that is supplied with the challenge. The predicted_fitted scores should be
    ...
    Args:
        ...
        prediction_file_csv (str): Name of the csv file scoring predicted_fitted scores
    """
    ...
    with open (args.output_file, "a") as f:
        ...
        csv_writer.writerow([prediction_file_csv.split("/")[-1].replace(".csv","_fitted"), error, std, p_corr, s_corr])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "output_file", help="csv file containing the predicted intelligibilities"
    )

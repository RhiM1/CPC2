...
class Minerva(torch.nn.Module):
    ...
    def forward(self, X: Tensor, D: Tensor, r: Tensor, p = None):
        ...
        a = torch.matmul(
            ...
            nn.functional.normalize(D, dim = 1).t()
        )
        # print(f"s:\n{a}\n")
...
class MetricPredictorSimple(nn.Module):
    ...
    def __init__(
        self, feat_dim = 768, hidden_size = 512, activation = nn.LeakyReLU
    ):
        super().__init__()
...
class ExSimple_learn(nn.Module):
    """Metric estimator for enhancement training.
    ...
    def __init__(
        ...
        self.use_g = False if linear_dim is None else True
        ...
        if ex_correct is not None:
            self.r = nn.Parameter(ex_correct, requires_grad = train_ex_class)

        linear_dim = feat_dim if linear_dim is None else linear_dim

        if linear_dim is not None:
            self.g = nn.Linear(in_features = feat_dim, out_features = linear_dim)

        self.f = Minerva(p_factor)

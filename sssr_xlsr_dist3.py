...
from models.wav2vec2_wrapper import Wav2Vec2Wrapper_no_helper
...
rresampler = T.Resample(44100, 16000)
resampler2 = T.Resample(44100, 16000)
...
def dataio_prep(json_path,data_root):
    ...
    def noisy_pipeline(noisy_wav):
        in_wav = sb.dataio.dataio.read_audio(data_root+"/scenes/"+ noisy_wav +"_mixed_CH0.wav")
        print(in_wav.shape)
        #sum stero channels 
        in_wav = in_wav[:,0]
...
with open("CPC1_train_xlsr2.csv","w") as f:

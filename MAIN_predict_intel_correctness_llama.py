...
def validate_model(model,test_data,optimizer,criterion,N):
    ...
    prompt_list = test_data["prompt"]
    #haspi_list = test_data["haspi"]
    #print(name_list)
    running_loss = 0.0
    loss_list = []
    test_dict = {}
    for sub,name,corr,scene,lis,prompt in  zip(subset_list,name_list,correctness_list,scene_list,listener_list,prompt_list):
        test_dict[name] = {"prompt":prompt,"subset":sub,"signal": name,"correctness":corr,"scene": scene,"listener":lis}
    ...
    for batch in tqdm(my_dataloader):
        ...
        output,_ = model(prompt,feats_l.float())
...
def test_model(model,test_data,optimizer,criterion,N):
    ...
    test_dict = {}
    for sub,name,corr,scene,lis,prompt in  zip(subset_list,name_list,correctness_list,scene_list,listener_list,prompt_list):
        test_dict[name] = {"subset":sub,"signal": name,"correctness":corr,"scene": scene,"listener":lis,"prompt":prompt}
...
def train_model(model,train_data,optimizer,criterion,N):
    ...
    train_dict = {}
    for sub,name,corr,scene,lis,prompt in  zip(subset_list,name_list,correctness_list,scene_list,listener_list,prompt_list):
        train_dict[name] = {"subset":sub,"signal": name,"correctness":corr,"scene": scene,"listener":lis,"prompt":prompt}
    ...
    #train_set.set_output_keys(["wav","clean_wav", "formatted_correctness","audiogram_np","haspi"])
    train_set.set_output_keys(["wav","formatted_correctness","prompt"])
    ...
    for batch in tqdm(my_dataloader, total=len(my_dataloader)):
        ...
        #output_r,_ = model(feats_r.float())
        output,_ = model(prompt,feats_l.float())
        print(output,target_scores)
        #print(output_l.shape,output_r.shape)
        #print(output_l)
        #print(output_r)
...
def main(args):
    ...
    if args.model == "XLSREncoder":
        ...
        from models.ni_predictors import LlamaHuBERTMetricPredictor
        model = LlamaHuBERTMetricPredictor().to("cuda:0")
    else:
    ...
    else: #we don't have the test data so we will use the val data
        test_data = val_data
